...
LexerEntry::LexerEntry(int tokenType, const std::string &symbol)
    : symbol(symbol), tokenType(tokenType) {
}

void Lexer::insertSymbol(const std::string &symbol, int tokenType) {
    entries.emplace_back(tokenType, symbol);
}

void Lexer::printTable() const {
    std::cout << "符号表：\n";
...
void Lexer::handleAlpha(const std::string &code, size_t &index) {
    size_t end = index + 1;
...
void Lexer::handleDigit(const std::string &code, size_t &index) {
    size_t end = index + 1;
...
bool Lexer::handleSeparatorOrOperator(const std::string &code, size_t &index) {
